...
class Unet_group(nn.Module):
      def __init__(self, n_classes,solid_name,n_element,gc_layer,pr_model,pretrain):
          ...
          if pr_model=='mobilenet_v2':
             for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(1280,layer_n,solid_name,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,solid_name, n_element)
             ...
             if not gc_layer[-1]==1280:
              ...
              for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(512,layer_n,solid_name,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,solid_name,n_element)
                 gc_block.append(layer)
              self.gc_model=nn.Sequential(*gc_block)
              ...
              for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(2048,layer_n,solid_name,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,solid_name,n_element)
                 gc_block.append(layer)
              self.gc_model=nn.Sequential(*gc_block)
...
class Unet_groupcat(nn.Module):
      def __init__(self, n_classes,solid_name,n_element,gc_layer,pr_model='mobilenet_v2'):
          ...
          if pr_model=='mobilenet_v2':
              for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(1280,layer_n,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,n_element)
                 gc_block.append(layer)
              self.gc_model=nn.Sequential(*gc_block)
              ...
              for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(512,layer_n,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,n_element)
                 gc_block.append(layer)
              self.gc_model=nn.Sequential(*gc_block)
              ...
              for (i, layer_n) in enumerate(gc_layer):
                 if i==0:
                     layer=Groupmulticonv(2048,layer_n,n_element)
                 else: 
                     layer=Groupmulticonv(gc_layer[i-1],layer_n,n_element)
                 gc_block.append(layer)
              self.gc_model=nn.Sequential(*gc_block)
      ...
      def forward(self,x):
         ...
         #[b,512,60, 14,14]
          x6=x6.permute(0,2,1,3,4)
        #[b,60,512,14,14]
          x6=x6.view(-1,x6.shape[2],x6.shape[3],x6.shape[4]) #not sure the order
        #[b*60,512,14,14]
