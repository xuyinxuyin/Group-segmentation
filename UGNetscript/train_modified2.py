...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 13 16:53:27 2020
...
from unet_modified_new import UNet
from unet_modified_new import UNet_mv
from unet_modified_new import Pool_UNet_mv
from unet_modified_new import Catgroup_UNet
from unet_modified_new import Unet_mv
from unet_modified_new import Unet_group
from unet_modified_new import Unet_pool
from unet_modified_new import Unet_groupcat
...
from config_defaults_new import _C as cfg
import math
...
def check_arguments(cfg, args):
    ...
    if cfg.CHOOSE_MODEL in ['group', 'groupcat']:
        ...
        assert len(cfg.GC_FILTER_SIZE)==len(cfg.GC_LAYER)
...
def train_net(net,
    ...
    if solid_name=='cir':
        check_exi_path='ugnetcheckpoint_'+str(nol)+'_'+str(nov)+'_'+str(resolui)+'_'+str(resoluj)+'_'+str(angle)+'_'+str(ypropx)+'_'+choose_model+'_'+str(pretrain)+'_'+'_'.join(map(str,gc_filter))
        log_path='log_'+str(nol)+'_'+str(nov)+'_'+str(resolui)+'_'+str(resoluj)+'_'+str(angle)+'_'+str(ypropx)+'_'+choose_model+'_'+str(pretrain)+'_'+'_'.join(map(str,gc_filter))
    else:
        check_exi_path='ugnetcheckpoint_'+str(solid_name)+'_'+str(resolui)+'_'+str(angle)+'_'+choose_model+'_'+str(pretrain)+'_'+'_'.join(map(str,gc_filter))
        log_path='log_'+str(solid_name)+'_'+str(resolui)+'_'+str(angle)+'_'+choose_model+'_'+str(pretrain)+'_'+'_'.join(map(str,gc_filter))
    ...
    if not skip_train:
        for epoch in range(start_epoch, epochs):
            ...
            if save_cp:                 ###different from train_modified, save latest epoch after each iteration
                try:
                    os.mkdir(check_exi_path)
                    logging.info('Created checkpoint directory')
                except OSError:
...
if __name__ == '__main__':
    ...
    if cfg.PR_MODEL=='custom':
        ...
        if cfg.CHOOSE_MODEL=='group':
            net=Unet_group(n_classes=15,solid_name=cfg.SOLID,n_element=cfg.N_ELEMENT,gc_filter=cfg.GC_FILTER_SIZE, gc_layer=cfg.GC_LAYER,pr_model=cfg.PR_MODEL,pretrain=cfg.PRETRAIN)
        elif cfg.CHOOSE_MODEL=='mv':
            ...
            net=Unet_groupcat(n_classes=15,solid_name=cfg.SOLID,n_element=cfg.N_ELEMENT,gc_filter=cfg.GC_FILTER_SIZE,gc_layer=cfg.GC_LAYER, pr_model=cfg.PR_MODEL,pretrain=cfg.PRETRAIN)
