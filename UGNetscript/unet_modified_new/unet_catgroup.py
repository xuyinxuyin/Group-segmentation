...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 28 20:53:35 2020
...
class Catgroup_UNet(nn.Module):
    def __init__(self, n_channels, n_classes,gc_layer=[512,512],n_element=60,layer_mul=1,bilinear=True):
        ###maybe change the middle layers in gc_block, adding parameters like [512,512,512]
        super(Catgroup_UNet, self).__init__()
        ...
        for (i, layer_n) in enumerate(gc_layer):
            if i==0:
                layer=Groupmulticonv(256*layer_mul,layer_n*layer_mul,n_element)
            else:
                layer=Groupmulticonv(gc_layer[i-1]*layer_mul,layer_n*layer_mul,n_element)
    ...
    def forward(self, x):
        ...
        x5 = self.down4(x4)
        # [n*60,512,14,14]
        x6= x5.view(-1, self.n_element, x5.shape[1], x5.shape[2], x5.shape[3]) 
